...
<!--            <a-icon slot="suffixIcon" type="sync" />-->
          ...
          <henpay-text-up v-model:value="vdata.searchData.mchNo" :placeholder="'商户号'" />
          <henpay-text-up v-model:value="vdata.searchData.isvNo" :placeholder="'服务商号'" />
          <henpay-text-up v-model:value="vdata.searchData.appId" :placeholder="'应用AppId'" />
...
function initPayWay() {
  req.list<{
    ...
    records: any[],
    total: number
  }>(API_URL_PAYWAYS_LIST, { pageSize: -1 }).then((res) => {
    // 支付方式下拉列表
